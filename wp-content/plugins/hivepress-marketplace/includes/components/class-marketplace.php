...
final class Marketplace extends Component {
	...
	public function __construct( $args = [] ) {
		...
		if ( is_admin() ) {
			...
			add_action( 'template_redirect', [ $this, 'redirect_product_page' ] );
	...
	public function validate_message( $errors, $message ) {
		if ( empty( $errors ) && get_option( 'hp_order_message_restriction' ) ) {
			...
			if ( $listing && $this->is_sellable( $listing ) ) {
				...
				if ( $product_id && ! wc_customer_bought_product( '', $message->get_sender__id(), $product_id ) ) {
	...
	public function alter_listing_view_page( $blocks, $template ) {
		...
		if ( $listing && $this->is_sellable( $listing ) && ! is_null( $listing->get_price() ) && ( ! get_option( 'hp_listing_allow_quantity' ) || $listing->get_quantity() !== 0 ) ) {
